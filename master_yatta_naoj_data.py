...
for name in psfnames:
    s = name.split('/')[-1].split('HSC-I')
    tp = s[1].split('-')
    tract = tp[1]
    patch = tp[2]
            ...
            if os.path.isfile(dname):
                ...
                if coord in tractpatch:
                    ...
                    if os.path.isfile(psfname):
                        ...
                        psfimhdu = pyfits.ImageHDU(data=psfhdu[0].data.copy())
    
                        hdulist = pyfits.HDUList([phdu, psfimhdu])
                        hdulist.writeto('%s/%s_%s_psf.fits'%(config['datadir'], name, band), clobber=True)
